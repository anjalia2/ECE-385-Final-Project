...
void setColorPalette (uint8_t color, uint8_t red, uint8_t green, uint8_t blue)
{
	...
	//uint8_t shift = (color % 2);
	//hdmi_ctrl->palette[index] = (green << 4) | (blue << 1);
	//hdmi_ctrl->palette[index+1] = red;

	//hdmi_ctrl->palette[color] = (((red) << 8) | ((green)  << 4) | (blue));
	//hdmi_ctrl->palette[color/2]
	uint32_t previous = hdmi_ctrl->palette[color/2];
	if(color % 2 == 0){
		uint32_t top = previous & 0xFFFF0000;
		hdmi_ctrl->palette[color/2] = ((top) | ((red) << 8) | ((green)  << 4) | (blue));

	} else {
		uint32_t bottom = previous & 0x0000FFFF;
		hdmi_ctrl->palette[color/2] = (((red) << 24) | ((green)  << 20) | (blue << 16) | bottom);
	}
}

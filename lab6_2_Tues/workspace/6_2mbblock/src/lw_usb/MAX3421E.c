...
void MAXreg_wr(BYTE reg, BYTE val) {
	...
	int transfer = XSpi_Transfer(&SpiInstance, writeBuffer, NULL, 2);
	if(transfer != 0){
		xil_printf("error");
	}
...
BYTE* MAXbytes_wr(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	for(int count = 0; count < nbytes; count++){
		writeBuffer[count+1] = data[count];
	}
	int transfer = XSpi_Transfer(&SpiInstance, writeBuffer, NULL, nbytes+1);
	if(transfer != 0){
		xil_printf("error");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
...
BYTE MAXreg_rd(BYTE reg) {
	...
	u8 writeBuffer[1];
	writeBuffer[0] = reg;
	int transfer = XSpi_Transfer(&SpiInstance, writeBuffer , readBuffer, 2);
	if(transfer != 0){
		xil_printf("error");
	}
	XSpi_SetSlaveSelect(&SpiInstance, 0);
...
BYTE* MAXbytes_rd(BYTE reg, BYTE nbytes, BYTE* data) {
	...
	u8 writeBuffer[1];
	writeBuffer[0] = reg;
	int transfer = XSpi_Transfer(&SpiInstance, writeBuffer , readBuffer, nbytes+1);
	if(transfer != 0){
		xil_printf("error");
	}
	for(int count = 0; count < nbytes; count++){
